...
export default function ScoreRadar({ subScores }) {
  ...
  const data = Object.entries(LABELS).map(([key, label]) => ({
    subject: label,
    score: subScores[key] !== null && subScores[key] !== undefined ? subScores[key] : null,
    fullMark: 100,
  }));
  ...
  return (
    <div style={{ width: '100%', position: 'relative' }}>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data}>
            ...
            <Tooltip
              ...
              formatter={(value) => [value !== null ? `${Math.round(value)} / 100` : 'Unavailable', 'Dimension Score']}
            />
          </RadarChart>
        </ResponsiveContainer>
      </div>
      ...
      <table className="sr-only">
        <caption>Resume Analysis Sub-scores</caption>
        <thead>
          <tr>
        ...
        <tbody>
          {data.map((item) => (
            <tr key={item.subject}>
              <td>{item.subject}</td>
              <td>{item.score !== null ? `${Math.round(item.score)} out of 100` : 'Unavailable'}</td>
            </tr>
          ))}
        </tbody>
      </table>
